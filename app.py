...
def load_overall_analysis():
    ...
    with col5:    
        ...
        st.plotly_chart(fig)
    with col8:
        st.subheader("India's Startups Hubs")
        sel=st.selectbox('',['Top 5','ALL'])
        Top_cities=df.drop_duplicates(subset=['Startup Name'])
        if sel=='Top 5':
            check=Top_cities.groupby('City')['Startup Name'].count().sort_values(ascending=False).head()
        else:
             check=Top_cities.groupby('City')['Startup Name'].count()
        trace1=go.Bar(x=check.index,y=check.values)
        data=[trace1]
        layout=go.Layout(title='')
        fig=go.Figure(data,layout)
        st.plotly_chart(fig)
    ...
    with col6:
        ...
        top_5=df.groupby('Startup Name')['amount'].sum().sort_values(ascending=False).head()
        trace1=go.Bar(x=top_5.index,y=top_5.values)
        data=[trace1]
        layout=go.Layout(title='')
        fig=go.Figure(data,layout)
        st.plotly_chart(fig)
    with col7:
        ...
        top_Industries=df.groupby('Industry Vertical')['amount'].sum().sort_values(ascending=False).head()
        trace1=go.Pie(labels=top_Industries.index,values=top_Industries.values)
        data=[trace1]
        layout=go.Layout(title='')
        fig=go.Figure(data,layout)
        st.plotly_chart(fig)
